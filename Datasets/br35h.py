"""
Br35H Brain MRI Dataset Loader
"""
...
_CLASSNAMES = ["01"]  # Brain MRI has only one class
...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_image_data(self):
        ...
        for classname in self.classnames_to_use:
            # Normal images from 'no' folder
            normal_dir = os.path.join(self.source, 'no')
            ...
            # Abnormal images from 'yes' folder with masks from Br35H-Mask-RCNN
            abnormal_dir = os.path.join(self.source, 'yes')
            mask_dir = os.path.join(self.source, 'Br35H-Mask-RCNN', self.split.value)
            
            if os.path.exists(abnormal_dir):
                abnormal_files = sorted([f for f in os.listdir(abnormal_dir) if f.endswith(('.jpg', '.png'))])
                
                for img_file in abnormal_files:
                    image_path = os.path.join(abnormal_dir, img_file)
