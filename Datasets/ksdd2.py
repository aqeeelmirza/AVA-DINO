...
_CLASSNAMES = ["01"]  # KSDD2 has only one class
...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_image_data(self):
        ...
        for classname in self.classnames_to_use:
            ...
            # Images: don't end with _GT.png
            # Masks: end with _GT.png
            image_files = [f for f in all_files if not f.endswith('_GT.png')]
            mask_files = {f.replace('_GT.png', '.png'): f for f in all_files if f.endswith('_GT.png')}
            ...
            for image_file in image_files:
                ...
                if image_file in mask_files:
                    # Anomaly image
                    anomaly = "Anomaly"
                    mask_path = os.path.join(split_dir, mask_files[image_file])
                else:
                    # Normal image
                    anomaly = "Normal"
                    mask_path = None
